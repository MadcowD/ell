...
from ell2a.types.message import Any, Any, Field, Message, Optional
...
class SerializedLMPUses(SQLModel, table=True):
    ...
    lmp_user_id: Optional[str] = Field(default=None, foreign_key="serializedlmp.lmp_id",
                                       primary_key=True, index=True)  # ID of the LMP that is being used
...
class InvocationContentsBase(SQLModel):
    ...
    global_vars: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSON))
    free_vars: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column(JSON))
...
class Invocation(InvocationBase, table=True):
    ...
    __table_args__ = (
        ...
        Index('ix_invocation_created_at_latency_ms',
              'created_at', 'latency_ms'),
        Index('ix_invocation_created_at_tokens', 'created_at',
