from ell2a.studio.datamodels import InvocationsAggregate
from typing import Optional, Dict, Any

from sqlmodel import Session
from ell2a.stores.sql import PostgresStore, SQLiteStore
from ell2a import __version__
from fastapi import FastAPI, Query, HTTPException, Depends, Response, WebSocket, WebSocketDisconnect
from fastapi.middleware.cors import CORSMiddleware
import logging
import json
from ell2a.studio.config import Config
from ell2a.studio.connection_manager import ConnectionManager
from ell2a.studio.datamodels import InvocationPublicWithConsumes, SerializedLMPWithUses

from ell2a.types import SerializedLMP
...
def create_app(config: Config):
    ...
    app = FastAPI(title="ell2a Studio", version=__version__)
    ...
    def get_invocation(
        ...
        invocation = serializer.get_invocations(
            session, lmp_filters=dict(), filters={"id": invocation_id})[0]
        return invocation
    ...
    def get_blob(
        ...
        if serializer.blob_store is None:
            raise HTTPException(
                status_code=400, detail="Blob storage is not configured")
        try:
    ...
    def get_lmp_history(
        # Default to 1 year, max 10 years
    ...
    def get_invocations_aggregate(
        ...
        aggregate_data = serializer.get_invocations_aggregate(
            session, lmp_filters=lmp_filters, days=days)
