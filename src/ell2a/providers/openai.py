        ...
        def translate_to_provider(self, ell_call : EllCallParams) -> Dict[str, Any]: 
            ...
            for message in ell_call.messages:
                if (tool_calls := message.tool_calls):
                    ...
                    assert all(t.tool_call_id for t in tool_calls), "Agent calls must have tool call ids."
                    ...
                    for tool_result in tool_results:
                        assert all(cb.type == "text" for cb in tool_result.result), "Agent result does not match expected content blocks."
                        openai_messages.append(dict(
                            role="tool",
