...
def _no_api_key_warning(model, client_to_use: Optional[Any], name=None,  long=False, error=False):
    color = Fore.RED if error else Fore.LIGHTYELL2AOW_EX
    prefix = "ERROR" if error else "WARNING"
    # openai default
    client_to_use_name = client_to_use.__class__.__name__ if (
        client_to_use) else "OpenAI"
    client_to_use_module = client_to_use.__class__.__module__ if (
        client_to_use) else "openai"
...
* Or, specify a client explicitly in the decorator:
    ...
    @ell2a.simple(model="{model}", client={client_to_use_name}(api_key=your_api_key))
    def your_lmp_name(...):
        ...
    ```
* Or explicitly specify the client when calling the LMP:
    ...
    ```
""" if long else " at time of definition. Can be okay if custom client specified later! https://docs.agentbase.space/core_concepts/models_and_api_clients.html ") + f"{Style.RESET_ALL}"
...
def _warnings(model, fn, default_client_from_decorator):
    ...
    if not default_client_from_decorator:
        ...
        if model not in config.registry:
            logger.warning(f"""{Fore.LIGHTYELL2AOW_EX}WARNING: Model `{model}` is used by LMP `{fn.__name__}` but no client could be found that supports `{model}`. Defaulting to use the OpenAI client `{config.default_client}` for `{model}`. This is likely because you've spelled the model name incorrectly or are using a newer model from a provider added after this ell2a version was released.

* If this is a mistake either specify a client explicitly in the decorator:
```python
import ell2a
ell2a.simple(model, client=my_client)
def {fn.__name__}(...):
...
```
...
{Style.RESET_ALL}""")
            ...
            logger.warning(_no_api_key_warning(
                model, fn.__name__, client_to_use, long=False))


def _autocommit_warning():
    if (config.get_client_for("gpt-4o-mini")[0] is None):
        logger.warning(f"{Fore.LIGHTYELL2AOW_EX}WARNING: Autocommit is enabled but no OpenAI client found for autocommit model 'gpt-4o-mini' (set your OpenAI API key). Commit messages will not be written.{Style.RESET_ALL}")
        return True
    return False
