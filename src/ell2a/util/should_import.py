...
def should_import(module_name: str, raise_on_error: bool = False) -> bool:
    ...
    """
    if module_name.startswith("ell2a"):
        return True
    try:
        try:
        ...
        if spec.has_location:
            ...
            site_packages = list(site.getsitepackages()) + (list(site.getusersitepackages())
                                                            if isinstance(site.getusersitepackages(), list) else [site.getusersitepackages()])

            additional_paths = [Path(p).resolve() for p in sys.path if Path(
                p).resolve() not in map(Path, site_packages)]

            project_root = Path(os.environ.get(
                "ELL2A_PROJECT_ROOT", os.getcwd())).resolve()
            ...
            additional_paths = [
                path for path in additional_paths if path != cwd]
