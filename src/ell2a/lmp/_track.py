...
import logging
import threading
from ell2a.types import SerializedLMP, Invocation, InvocationTrace, InvocationContents
from ell2a.types.studio import LMPType, utc_now
from ell2a.util._warnings import _autocommit_warning
import ell2a.util.closure
...
from ell2a.util.serialization import compute_state_cache_key
from ell2a.util.serialization import prepare_invocation_params
...
def _track(func_to_track: Callable, *, forced_dependencies: Optional[Dict[str, Any]] = None) -> Callable:
    ...
    lmp_type = getattr(func_to_track, "__ell2a_type__", LMPType.OTHER)

    # see if it exists
    if not hasattr(func_to_track, "_has_serialized_lmp"):
        func_to_track._has_serialized_lmp = False

    if not hasattr(func_to_track, "__ell2a_hash__") and not config.lazy_versioning:
        ell2a.util.closure.lexically_closured_source(
    ...
    def tracked_func(*fn_args, _get_invocation_id=False, **fn_kwargs) -> str:
        ...
        try:
            ...
            try_use_cache = hasattr(
                func_to_track.__wrapper__, "__ell2a_use_cache__")

            if try_use_cache:
                # Todo: add nice logging if verbose for when using a cahced invocaiton. IN a different color with thar args..
                if not hasattr(func_to_track, "__ell2a_hash__") and config.lazy_versioning:
                    fn_closure, _ = ell2a.util.closure.lexically_closured_source(
                        func_to_track)
                ...
                cache_store = func_to_track.__wrapper__.__ell2a_use_cache__
                cached_invocations = cache_store.get_cached_invocations(
                    func_to_track.__ell2a_hash__, state_cache_key)
            ...
            completion_tokens = usage.get(
                "completion_tokens", 0) if usage else 0
    ...
    if hasattr(func_to_track, "__ell2a_api_params__"):
        tracked_func.__ell2a_api_params__ = func_to_track.__ell2a_api_params__
    if hasattr(func_to_track, "__ell2a_params_model__"):
        tracked_func.__ell2a_params_model__ = func_to_track.__ell2a_params_model__
    tracked_func.__ell2a_func__ = func_to_track
    tracked_func.__ell2a_track = True
...
def _serialize_lmp(func):
    ...
    for f in func.__ell2a_uses__:
        _serialize_lmp(f)

    if getattr(func, "_has_serialized_lmp", False):
        return
    func._has_serialized_lmp = False
    fn_closure = func.__ell2a_closure__
    lmp_type = func.__ell2a_type__
    name = func.__qualname__
    api_params = getattr(func, "__ell2a_api_params__", None)

    lmps = config.store.get_versions_by_fqn(fqn=name)
    version = 0
    already_in_store = any(lmp.lmp_id == func.__ell2a_hash__ for lmp in lmps)
    ...
    if not already_in_store:
        ...
        config.store.write_lmp(
            serialized_lmp, [f.__ell2a_hash__ for f in func.__ell2a_uses__])
...
def _write_invocation(func, invocation_id, latency_ms, prompt_tokens, completion_tokens,
    ...
    invocation_contents = InvocationContents(
        ...
        global_vars=get_immutable_vars(func.__ell2a_closure__[2]),
        free_vars=get_immutable_vars(func.__ell2a_closure__[3])
    )
