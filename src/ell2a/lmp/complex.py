...
from ell2a.util._warnings import _no_api_key_warning, _warnings
from ell2a.util.verbosity import compute_color, model_usage_logger_pre
...
3. Comprehensive Integration:
    - Integrates with ell2a's tracking system for monitoring LMP versions, usage, and performance.
...
.. code-block:: python
    ...
    def generate_story(prompt: str) -> List[Message]:
        ...
        return [
            ell2a.user(f"Write a short story based on this prompt: {prompt}")
        ]

    story : ell2a.Message = generate_story("A robot discovers emotions") 
...
.. code-block:: python
    ...
    conversation = [
        ell2a.user("Hello, who are you?"),
        ell2a.assistant("I'm an AI assistant. How can I help you today?"),
        ell2a.user("Can you explain quantum computing?")
    ]
    response : ell2a.Message = chat_bot(conversation)
...
.. code-block:: python
    ...
    conversation = [
        ell2a.user("What's the weather like in New York?")
    ]
...
.. code-block:: python
    ...
    @ell2a.complex(model="gpt-4", response_format=PersonInfo)
    def extract_person_info(text: str) -> List[Message]:
        return [
            ell2a.system("Extract person information from the given text."),
            ell2a.user(text)
        ]

    text = "John Doe is a 30-year-old software engineer."
    result : ell2a.Message = extract_person_info(text)
...
.. code-block:: python
    ...
    def describe_image(image: PIL.Image.Image) -> List[Message]:
        return [
            ell2a.system("Describe the contents of the image in detail."),
            ell2a.user([
