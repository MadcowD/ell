...
simple.__doc__ = """The fundamental unit of language model programming in ell2a.
  ...
  Usage:
  The decorated function can return either a single prompt or a list of ell2a.Message objects:

  .. code-block:: python

      @ell2a.simple(model="gpt-4", temperature=0.7)
      ...
      @ell2a.simple(model="gpt-4", temperature=0.7)
      def describe_image(image : PIL.Image.Image) -> List[ell2a.Message]:
          '''Describe the contents of an image.''' # unused because we're returning a list of Messages
          return [
              # helper function for ell2a.Message(text="...", role="system")
              ell2a.system("You are an AI trained to describe images."),
              # helper function for ell2a.Message(content="...", role="user")
