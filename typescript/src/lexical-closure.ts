...
function lexicalClosure(
  ...
  const { dependencies, imports } = processDependencies(
    func,
    globals,
    frees,
    alreadyClosed,
...
function getGlobalsAndFrees(func: Function): {
...
function processVariable(
  ...
  if (typeof varValue === 'function' && !FORBIDDEN_NAMES.includes(varName)) {
    try {
      const { fullSource, uses: depUses } = lexicalClosure(
        varValue,
        alreadyClosed,
        false,
        [...recursionStack]
...
function buildFinalSource(
  ...
  const separatedDependencies = [
    ...new Set([...imports, ...moduleSrc, ...dependencies, source]),
  ]
  return `${DELIM}\n${separatedDependencies.join(`\n${DELIM}\n`)}\n${DELIM}\n`
}

function cleanSrc(dirtySrc: string): string {
  const sections = dirtySrc
    .split(DELIM)
...
export function lexicallyClosuredSource(
  func: Function
): [string, string, any, any, Set<string>] {
  const { functionSource, dependenciesSource, uses } = lexicalClosure(
    func,
    new Set(),
    true
  )
