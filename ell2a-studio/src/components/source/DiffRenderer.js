...
export function DiffRenderer({ previousCode, code, stylesheet, useInlineStyles, startingLineNumber, commonProps }) {
                ...
                <div key={`${partIndex}-${lineIndex}`} style={{ display: 'flex', backgroundColor }}>
                    ...
                    <span
                        style={{
                            ...
                            marginRight: '15px',
                            marginLeft: '5px',
                            color: added ? '#2cbe4e' : removed ? '#f85149' : '#6e7681'
                        }}>
                        {diffIndicator}
                    </span>
                    <span style={{ flex: 1, paddingLeft: '15px' }}>{lineContent}</span>
                </div>
            )
