...
const BoundedVariableWrapper = ({ children, selectedInvocation, content, initial_global_vars, initial_free_vars }) => {
    ...
    const formattedValue = `${var_name} = ${JSON.stringify(value)?.replace(
        /"<Object of type ([^>]+)>"/g,
        '<Object of type $1>'
    )}`
    ...
    return (
        <div className="relative rounded border border-gray-500 mt-2 py-2">
            <span
                className="absolute -top-2 left-2 bg-gray-800 text-[0.6rem] text-gray-400"
                style={{ backgroundColor: 'rgb(28, 31, 38)' }}>
                {isGlobal ? 'mutable globalvar' : isFree ? 'freevar' : 'unknown'}{' '}
                {!selectedInvocation ? 'value at lmp definition' : `value at ${selectedInvocation.id}`}
            </span>
            <div className="ml-5">
                <CodeHighlighter
                    code={formattedValue}
...
const LMPSourceView = ({
    ...
    const dependencies = useMemo(() => {
        ...
        const procd_deps = unprocessedDependencies
            .split('\n')
            .map((line) => {
