...
const typeMatchers = {
    AgentResult: (data) => data && typeof data === 'object' && 'agent_call_id' in data && 'result' in data,
    AgentCall: (data) => data && typeof data === 'object' && 'agent' in data && 'params' in data,
    ContentBlock: (data) =>
        data &&
        typeof data === 'object' &&
        ['text', 'image', 'audio', 'agent_call', 'parsed', 'agent_result'].some((key) => key in data),
...
const preprocessData = (data, currentLevel = 0, typeMatchLevel = 0) => {
    if (currentLevel >= typeMatchLevel) {
        if (typeMatchers.Message(data)) {
            ...
            if (content.length === 1) {
                ...
                if (block.agent_call) {
                    return { type: 'Message', role, content: preprocessData(block.agent_call) }
                }
                const complexType = ['image', 'audio', 'parsed', 'agent_result'].find((type) => block[type])
                if (complexType) {
        ...
        if (typeMatchers.ContentBlock(data)) {
            const contentType = Object.keys(data).find((key) =>
                ['text', 'image', 'audio', 'agent_call', 'parsed', 'agent_result'].includes(key)
            )
    ...
    if (typeof data === 'object' && data !== null) {
        ...
        return Object.fromEntries(
            Object.entries(data).map(([key, value]) => [key, preprocessData(value, currentLevel + 1, typeMatchLevel)])
        )
    }
    ...
    if (typeof data === 'object' && data !== null && 'type' in data) {
        ...
        return (
            <span className="text-gray-300">
                ...
                {type === 'Message' ? (
                    <>
                        {renderInline(rest.content, customRenderers)},<span className="text-sky-300">role</span>=
                        <span className="text-green-300">"{rest.role}"</span>
                    </>
                ) : (
        ...
        if (isImage) {
            return (
                <img
                    src={data.content}
                    alt="PIL.Image"
                    style={{ display: 'inline-block', verticalAlign: 'middle', maxHeight: '1.5em' }}
                />
            )
        } else if (isNdarray) {
    ...
    try {
        ...
        return (
            <div className="text-sm font-mono">
                {inline
                    ? renderInline(preprocessedContent, customRenderers)
                    : renderNonInline(preprocessedContent, customRenderers)}
            </div>
        )
    } catch {
