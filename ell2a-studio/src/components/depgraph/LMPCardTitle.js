...
export function LMPCardTitle({
    ...
    return (
        <div
            className={`flex items-center space-x-2 ${paddingClass} ${scaleClass} transition-colors ${additionalClassName} ${hoverClass} ${cursorClass} rounded-md overflow-hidden`}
            {...(clickable ? rest : {})}>
            ...
            {nameOverride ? (
                ...
                <code
                    className={`px-2 py-1 rounded-md ${
                        lmp.is_lmp ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'
                    } text-${fontSize} font-medium truncate`}>
                    {lmp.name}()
                </code>
            )}
            {displayVersion && (
                <VersionBadge version={lmp.version_number + 1} lmpId={lmp.lmp_id} shortVersion={shortVersion} />
            )}
