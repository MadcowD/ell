...
import { FiCopy, FiInfo, FiList, FiX } from 'react-icons/fi'
        ...
        <ResizableSidebar onResize={handleResize} isExpanded={isExpanded}>
            <motion.div
                ...
                <div className="flex space-x-2 p-2 ml-2 bg-muted/30 border-b border-border">
                    {[{ name: 'I/O', icon: FiList }, ...(isNarrowForInfo ? [{ name: 'Info', icon: FiInfo }] : [])].map(
                        ({ name, icon: Icon }) => (
                            <button
                                key={name}
                ...
                <div className="flex flex-grow overflow-hidden">
                    <motion.div
                        className={`flex-grow overflow-y-auto ${
                            isNarrowForInfo || activeTab === 'Info' ? 'w-full' : 'w-2/3'
                        }`}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
