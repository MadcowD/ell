...
const InvocationDataPane = ({ invocation }) => {
    ...
    const hasKwargs = useMemo(() => {
        return (
            typeof invocation.contents?.params === 'object' &&
    ...
    const hasResults = useMemo(() => {
        return Array.isArray(invocation.contents?.results)
            ...
            : invocation.contents?.results !== null && invocation.contents?.results !== undefined
    ...
    return (
        <div className="flex-grow p-4 overflow-y-auto w-[fullpx] hide-scrollbar">
            {(hasKwargs || isExternalLoading) &&
                renderCodeSection('Input', invocation.contents?.params, inputExpanded, setInputExpanded, 1)}

            {(hasResults || isExternalLoading) &&
                renderCodeSection('Results', invocation.contents?.results, outputExpanded, setOutputExpanded, 0)}
        </div>
    )
}
