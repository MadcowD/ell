...
const InvocationsTable = ({
    ...
    pageSize,
    onSelectTrace,
    currentlySelectedTrace,
    omitColumns = [],
    expandAll = false
}) => {
    const navigate = useNavigate()

    const onClickLMP = useCallback(
        ({ lmp, id: invocationId }) => {
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            key: 'version',
            render: (item) => (
                <VersionBadge version={item.version} lmpId={item.lmp.id} className="text-xs scale-85 inline-block" />
            ),
            maxWidth: 150,
            sortable: true
        },
        {
            ...
            key: 'input',
            maxWidth: 400,
            render: (item) => <ContentsRenderer typeMatchLevel={1} item={item} field={'params'} />
        },
        {
            header: 'Output',
            key: 'output',
            render: (item) => <ContentsRenderer item={item} field={'results'} />,
            maxWidth: 600
        },
        {
            header: 'Start Time',
    ...
    return (
        <HierarchicalTable
            ...
            rowClassName={(item) => (item.id === currentlySelectedTrace?.id ? 'bg-blue-600 bg-opacity-30' : '')}
