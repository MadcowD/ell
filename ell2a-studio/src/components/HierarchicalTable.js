...
const TableRow = ({
    ...
    const { expandedRows, selectedRows, toggleRow, toggleSelection, isItemSelected, setHoveredRow, sortedData } =
                        ...
                        <React.Fragment key={index}>
                            <td
                                ...
                                <div
                                    ...
                                    <div ref={column.key === linkColumn ? primaryColumnRef : null}>{content}</div>
                                </div>
                            </td>
                        </React.Fragment>
                    )
        ...
        <thead>
            <tr className="text-left text-xs text-gray-400 border-t border-b border-l border-r border-gray-800 bg-gray-800/30">
                <th className="py-2 px-4 w-12">
                    <Checkbox checked={isAllSelected()} onCheckedChange={(checked) => toggleAllSelection(checked)} />
                </th>
                <th className="py-2 px-4 w-12">
                    <FiChevronRight className="text-gray-400 text-base" />
                </th>
                {schema.columns.map((column, index) => {
                    ...
                    return (
                        <React.Fragment key={index}>
                            <th
                                className={`py-2 px-4 whitespace-nowrap overflow-hidden text-ellipsis ${
...
const PaginationControls = ({ currentPage, totalPages, onPageChange, pageSize, totalItems }) => {
    ...
    return (
        <div className="flex justify-between items-center mt-4 text-sm">
            <div className="text-gray-400">{/* Showing {startItem} to {endItem} of {totalItems} items */}</div>
            <div className="flex items-center">
                <button
                ...
                <span className="mx-4 text-gray-400">Page {currentPage + 1}</span>
                <button
                    onClick={() => onPageChange(currentPage + 1)}
        ...
        <HierarchicalTableProvider
            ...
            <div className="overflow-x-auto hide-scrollbar relative" ref={tableRef}>
                <table className="w-full">
                    <TableHeader schema={filteredSchema} columnWidths={columnWidths} updateWidth={updateWidth} />
                    <TableBody
                ...
                <svg
                    ...
                    <LinkLines links={links} rowRefs={rowRefs} tableOffset={tableOffset} />
                </svg>
            </div>
            {onPageChange && (
                <PaginationControls
...
const LinkLines = ({ links, rowRefs, tableOffset }) => {
    ...
    const groupedLinks = useMemo(() => {
        const grouped = links?.reduce((acc, link) => {
            ...
            const isUnique = !acc[link.from].some(
                (existingLink) => existingLink.from === link.from && existingLink.to === link.to
            )
    ...
    return links?.map((link, index) => {
        ...
        if (startRow && endRow && startRow?.visible && endRow?.visible) {
            const isHighlighted = hoveredRow === link.from || hoveredRow === link.to
            const offset = (groupedLinks[link.from].indexOf(link) + 4) * 3 // Multiply by 20 for spacing
            const color = isHighlighted
                ? hoveredRow === link.from
                    ? '#f97316'
                    : '#3b82f6' // Orange if going from, Blue if coming to
                : '#4a5568'
