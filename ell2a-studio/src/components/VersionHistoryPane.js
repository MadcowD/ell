...
import { FiChevronRight, FiClock, FiCopy, FiGitCommit } from 'react-icons/fi'
                    ...
                    <div key={date} className="mb-6">
                        ...
                        {dateVersions.map((version) => {
                            ...
                            return (
                                <div
                                    key={version.lmp_id}
                                    className="mb-2 border border-gray-700 rounded-lg overflow-hidden">
                                    <div
                                        className={`bg-gray-800 p-3 flex items-center justify-between cursor-pointer hover:bg-gray-750 ${
                                            version.lmp_id === currentLmpId ? 'bg-blue-900' : ''
                                        }`}
                                        ...
                                        <div>
                                            ...
                                            {commitDetails && (
                                                <div className="text-sm text-gray-400 ml-6 mt-1 whitespace-pre-wrap">
                                                    {commitDetails}
                                                </div>
                                            )}
                                    ...
                                    <div
                                        className={`bg-gray-850 px-3 py-2 flex items-center justify-between ${
                                            version.lmp_id === currentLmpId || (totalIndex === 1 && !currentLmpId)
                                                ? 'bg-blue-800'
                                                : ''
                                        }`}>
