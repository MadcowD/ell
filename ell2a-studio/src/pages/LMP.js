...
import { useLMPs, useInvocationsFromLMP, useInvocation } from '../hooks/useBackend'
...
function LMP() {
    ...
    const requestedInvocation = useMemo(() => {
        if (!requestedInvocationQueryData) return invocations?.find((i) => i.id === requestedInvocationId)
    ...
    const sidebar = useMemo(
        () => <LMPDetailsSidePanel lmp={lmp} uses={uses} versionHistory={versionHistory} />,
    ...
    if (isLoadingLMP)
        return <div className="flex items-center justify-center h-screen bg-card text-card-foreground">Loading...</div>

    const omitColumns = ['name']

    return (
        <GenericPageLayout selectedTrace={selectedTrace} setSelectedTrace={setSelectedTrace} sidebarContent={sidebar}>
