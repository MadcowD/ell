...
import { FiFilter, FiPause, FiPlay, FiSearch } from 'react-icons/fi'
...
import { useInvocationsAggregate, useInvocationsFromLMP } from '../hooks/useBackend'
...
const Invocations = () => {
    ...
    const filteredInvocations = useMemo(() => {
        ...
        return invocations.filter((inv) => {
            ...
            const matchesAdvanced =
                (!advancedFilters.lmpName ||
                    inv.lmp.name.toLowerCase().includes(advancedFilters.lmpName.toLowerCase())) &&
                (!advancedFilters.inputContains ||
                    JSON.stringify(inv.contents?.params)
                        .toLowerCase()
                        .includes(advancedFilters.inputContains.toLowerCase())) &&
                (!advancedFilters.outputContains ||
                    inv.contents?.results.some((result) =>
                        JSON.stringify(result).toLowerCase().includes(advancedFilters.outputContains.toLowerCase())
                    )) &&
                ...
                (!advancedFilters.tokensMin ||
                    inv.prompt_tokens + inv.completion_tokens >= parseInt(advancedFilters.tokensMin)) &&
                (!advancedFilters.tokensMax ||
                    inv.prompt_tokens + inv.completion_tokens <= parseInt(advancedFilters.tokensMax))
    ...
    const sidebarMetrics = useMemo(() => {
        ...
        const successRate =
            (filteredInvocations.filter((inv) => inv.status === 'success').length / filteredInvocations.length) * 100
    ...
    const sidebarContent = (
        <InvocationsAnalyticsSidePanel aggregateData={aggregateData} sidebarMetrics={sidebarMetrics} />
    )
                ...
                {advancedFilters.isOpen && (
                    <div className="grid grid-cols-2 gap-4 bg-[#2d333b] p-4 rounded-md">
                        ...
                        <input
                            ...
                            onChange={(e) =>
                                setAdvancedFilters((prev) => ({ ...prev, outputContains: e.target.value }))
                            }
                        />
                        <div className="flex space-x-2">
                            <input
                                ...
                                onChange={(e) =>
                                    setAdvancedFilters((prev) => ({ ...prev, latencyMin: e.target.value }))
                                }
                            />
                            <input
                                type="number"
                                ...
                                onChange={(e) =>
                                    setAdvancedFilters((prev) => ({ ...prev, latencyMax: e.target.value }))
                                }
                            />
                        </div>
                        <div className="flex space-x-2">
