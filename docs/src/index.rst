...
.. raw:: html
   ...
   <a href="https://jobs.agentbase.space" target="_blank">
...
``ell2a`` is a lightweight prompt engineering library treating prompts as functions. After years of building and using language models at OpenAI and in the startup ecosystem, ``ell2a`` was designed from the following principles:
...
Prompts aren't just strings; they are all the code that leads to strings being sent to a language model. In ell2a, we think of one particular way of using a language model as a discrete subroutine called a **language model program** (LMP).
...
.. code-block:: python
   :emphasize-lines: 3,3
    ...
    ell2a.init(store='./logdir')  # Versions your LMPs and their calls
...
By forcing a functional decomposition of the problem, ``ell2a`` makes it **easy to implement test-time compute leveraged techniques in a readable and modular way.**
...
In addition to storing the source code of every LMP, ``ell2a`` optionally saves every call to a language model locally. This allows you to generate invocaiton datasets, compare LMP outputs by version, and generally do more with the full spectrum of prompt engineering artifacts.
...
Using ``@ell2a.simple`` causes LMPs to yield **simple string outputs.** But when more complex or multimodal output is needed, ``@ell2a.complex`` can be used to yield ``Message`` objects responses from language mdoels.
...
To get started with ``ell2a``, see the :doc:`Getting Started <getting_started>` section, or go onto :doc:`Installation <installation>` and get ell2a installed.
