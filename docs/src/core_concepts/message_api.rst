...
.. note::
   For a full API and usage refernece please refer to the :ref:`API Reference <api_reference>`.
...
.. autopydantic_model:: ell.types.message.Message()
...
This type and shape coercion is made possible through the ContentBlock's coerce method:

.. automethod:: ell.ContentBlock.coerce
...
Now let's see how we can do the same thing using ell's message API.
