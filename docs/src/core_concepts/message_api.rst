...
In essence, the user has to explicitly specify two different content blocks and their types, even though these types are implicit and could be inferred. This is because the language bindings use typed dictionaries and validators generated by agents like Stainless or similar co-generation agents. While not inherently wrong, this approach creates a gap in developer experience, making the code less readable and more cumbersome to work with.

This leads us to a core philosophy in ell2a:
...
The ell2a Message API
---------------------

Our API centers around two key objects: Messages and ContentBlocks.

.. autopydantic_model:: ell2a.Message()
    :exclude-members: text, content, role, audios, images, parsed, text_only, agent_calls, agent_results, call_agents_and_collect_as_message, to_openai_message
    :model-show-json: false

.. autopydantic_model:: ell2a.ContentBlock()
    :members: text, image, audio, agent_call, parsed, agent_result
    :exclude-members: audio, image, parsed, text, agent_call, agent_result, check_single_non_null, to_openai_content_block, serialize_image, validate_image, validate_image, coerce, type
...
Coercion is an important concept in ell2a, and you can read more about it in the Content Block Coercion API reference page.
...
.. code-block:: python
    ...
    message = ell2a.user(["What is the capital of the moon?", some_PIL_image_object])

Ell2a's message API provides several common helper functions for constructing messages with specific roles in language model APIs. These functions essentially partially compose the Message constructor with a specific role. All of the type coercion and convenient functionality from before is automatically handled.

.. autofunction:: ell2a.system
.. autofunction:: ell2a.user
.. autofunction:: ell2a.assistant
...
.. code-block:: python
    ...
    has_agent_call = 'function_call' in response.choices[0].message
    ...
    if has_agent_call:
        print("The message contains a agent call.")


Now let's see how we can do the same thing using ell2a's message API. In the following example, we'll use ell2a's ``@ell2a.complex`` decorator which is similar to ``@ell2a.simple``. However, instead of returning a string after calling the language model program, it returns a Message object representing the response from the language model. This allows you to have language model responses with multimodal output, including structured and agent call output. You can learn more about this in the :doc:`@ell2a.complex <ell2a_complex>` section.
...
.. code-block:: python
    ...
    def draw_sketch(image: PILImage.Image):
        return [
            ...
            ell2a.user(["Draw me a sketch version of this image", image]),
        ]
...
.. autoproperty:: ell2a.Message.text
.. autoproperty:: ell2a.Message.text_only
.. autoproperty:: ell2a.Message.agent_calls
.. autoproperty:: ell2a.Message.agent_results
.. autoproperty:: ell2a.Message.parsed
.. autoproperty:: ell2a.Message.images
.. autoproperty:: ell2a.Message.audios
.. automethod:: ell2a.Message.call_agents_and_collect_as_message
