...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Run all example scripts in the examples directory and its subdirectories.")
    parser.add_argument("-d", "--directory", default="../examples",
                        help="Root directory containing example scripts (default: ../examples)")
    ...
    parser.add_argument("--cache", action="store_true",
                        help="Use caching to skip previously successful runs")
...
def run_all_examples(args):
    examples_dir = os.path.abspath(os.path.join(
        os.path.dirname(__file__), args.directory))
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for example_path in example_files:
            future = executor.submit(
                run_example, example_path, args.verbose, cache)
        ...
        for future in as_completed(futures):
            ...
            if status == "SUCCESS":
                print(f"{Fore.GREEN}{futures[future]} . (Runtime: {
                      runtime:.2f}s){Style.RESET_ALL}")
            elif status == "CACHED":
                print(f"{Fore.BLUE}{futures[future]} C (Cached Runtime: {
                      runtime:.2f}s){Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}{futures[future]} F (Runtime: {
                      runtime:.2f}s){Style.RESET_ALL}")
                print(f"  Error: {error}")
                print(f"  Full output:")
                print(output)
                if not args.continue_on_error:
                    ...
                    for running_future in futures:
                        if not running_future.done():
                            print(f"{Fore.YELL2AOW}Cancelling: {
                                  futures[running_future]}{Style.RESET_ALL}")
    ...
    successful = sum(1 for _, status, _, _ in results if status in {
                     "SUCCESS", "CACHED"})
    ...
    print(f"{Fore.YELL2AOW}Skipped: {skipped}{Style.RESET_ALL}")

    if failed > 0:
        print("\nFailed examples:")
        for example, status, runtime, error in results:
            if status == "ERROR":
                print(f"{Fore.RED}{example} (Runtime: {
                      runtime:.2f}s){Style.RESET_ALL}")
                print(f"  Error: {error}")

    average_runtime = sum(runtime for _, _, runtime,
                          _ in results) / len(results)
    ...
    if all(status in {"SUCCESS", "CACHED"} for _, status, _, _ in results):
        ...
        print(f"\n{Fore.YELL2AOW}Some examples did not run successfully. Please review the output above.{
              Style.RESET_ALL}")
