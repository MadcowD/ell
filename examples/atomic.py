...
# atomic.py --manual  fro manual execution.
import argparse
import logging
import ell
...
def maybe_pause(manual_mode: bool, message: str):
    """
    Pause the execution and wait for user input if manual_mode is True.
    """
    if manual_mode:
...
def HandleError(subproblem: str, step: str, result: str, problem_tree: 'ProblemTree', retry_tracker: Dict[str, int], manual_mode: bool):
    """
    ...
    maybe_pause(manual_mode, f"Error encountered in step '{step}'. Handling error...")
    ...
    if current_retry < MAX_RETRIES:
        ...
        maybe_pause(manual_mode, f"Retrying step '{step}' ({retry_tracker[step]}/{MAX_RETRIES})")
        ...
        maybe_pause(manual_mode, f"Retried step '{step}'. Verifying new result...")
        ...
        maybe_pause(manual_mode, f"Verification for retried step '{step}': {'Valid' if is_valid else 'Invalid'}.")
        
        if is_valid:
            problem_tree.update_node(step, 'Solved')
        else:
            HandleError(subproblem, step, new_result, problem_tree, retry_tracker, manual_mode)
    else:
        problem_tree.update_node(subproblem, 'Failed')
        logger.error(f"Subproblem '{subproblem}' failed after {MAX_RETRIES} retries.")
        maybe_pause(manual_mode, f"Subproblem '{subproblem}' failed after {MAX_RETRIES} retries.")
...
def DecomposeToAtomicRecursive(subproblem: str, problem_tree: ProblemTree, retry_tracker: Dict[str, int], manual_mode: bool):
    """
    Recursively decompose a subproblem until all are atomic and add them to the problem tree.
    """
    if IsAtomic(subproblem):
        return
    logger.info(f"Decomposing subproblem: '{subproblem}'")
    maybe_pause(manual_mode, f"Decomposing subproblem: '{subproblem}'")
    ...
    logger.info(f"Decomposed subproblem '{subproblem}' into: {smaller_subproblems}")
    maybe_pause(manual_mode, f"Subproblem '{subproblem}' decomposed. Solving smaller subproblems...")
    
    for smaller_subproblem in smaller_subproblems:
        DecomposeToAtomicRecursive(smaller_subproblem, problem_tree, retry_tracker, manual_mode)
        SolveAtomicSubproblemRecursive(smaller_subproblem, problem_tree, retry_tracker, manual_mode)

def SolveAtomicSubproblemRecursive(subproblem: str, problem_tree: ProblemTree, retry_tracker: Dict[str, int], manual_mode: bool):
    """
    Recursively solve an atomic subproblem, handling decomposition if necessary.
    """
    if IsAtomic(subproblem):
        logger.info(f"Solving atomic subproblem: '{subproblem}'")
        maybe_pause(manual_mode, f"Defining steps for subproblem: '{subproblem}'")
        ...
        maybe_pause(manual_mode, f"Steps defined for subproblem: '{subproblem}'. Starting execution...")
        ...
        for step in steps:
            try:
                ...
                maybe_pause(manual_mode, f"Step '{step}' executed. Verifying result...")
                ...
                maybe_pause(manual_mode, f"Verification for step '{step}': {'Valid' if is_valid else 'Invalid'}.")
                
                if is_valid:
                    problem_tree.update_node(step, 'Solved')
                else:
                    HandleError(subproblem, step, result, problem_tree, retry_tracker, manual_mode)
            except Exception as e:
                logger.error(f"Exception occurred while solving step '{step}': {e}")
                problem_tree.update_node(step, 'Failed')
                maybe_pause(manual_mode, f"Step '{step}' failed with exception. Continuing...")
    else:
        logger.info(f"Subproblem '{subproblem}' is not atomic. Decomposing further.")
        maybe_pause(manual_mode, f"Decomposing subproblem: '{subproblem}'")
...
def SolveProblem(problem: str, manual_mode: bool) -> ProblemTree:
    """
    Orchestrate the entire problem-solving process.
    """
    logger.info(f"Starting to solve problem: '{problem}'")
    maybe_pause(manual_mode, "Decomposing the main problem...")
    ...
    logger.info(f"Decomposed problem into subproblems: {subproblems}")
    maybe_pause(manual_mode, "Problem decomposed. Creating problem tree...")
    ...
    for subproblem in subproblems:
        SolveAtomicSubproblemRecursive(subproblem, problem_tree, retry_tracker, manual_mode)
    
    logger.info(f"Final Problem Tree: {json.dumps(problem_tree.nodes, indent=2)}")
    maybe_pause(manual_mode, "Problem-solving complete.")
    return problem_tree
...
def main():
    ...
    try:
        ...
        parser = argparse.ArgumentParser(description="Problem Solving Framework with Groq and ell.")
        parser.add_argument(
            '--manual',
            action='store_true',
            help='Run the script in manual mode for step-by-step debugging.'
        )
        args = parser.parse_args()
        manual_mode = args.manual

        if manual_mode:
            logger.info("Manual mode enabled. The script will pause at each step for debugging.")
        ...
        # Example usage
        problem = "How many r are in the word Strawberry?"
        problem_tree = SolveProblem(problem, manual_mode)
